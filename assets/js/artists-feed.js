...
import {
  ...
  num, fmtSats, plural,
} from '/assets/js/publisher-card.js?v=ob-v181'
import { wirePublisherCards } from '/assets/js/publisher-card-actions.js?v=ob-v181'
...
export async function renderArtists({ panel, list, scope = 'global', lang = null, range = null, sort = null } = {}) {
  ...
  function paintNote() {
     ...
     * Global, all time, all languages, chart sort; see the shows-feed twin. */
    const charts = sortKey === 'chart' && !follows && !rangeDays(rangeKey)
      && (!langKey || langKey === LANG_ALL)
    mountFeedNote(panel,
